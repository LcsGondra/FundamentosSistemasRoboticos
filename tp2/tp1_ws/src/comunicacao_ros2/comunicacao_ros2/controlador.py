...
class Controlador(Node):
    ...
    def __init__(self):
        ...
        self.estado = 'parado'
        self.srv = self.create_service(SetBool, 'mudar_estado', self.mudar_estado_callback)
        self.sub_bateria = self.create_subscription(Float32, '/bateria', self.bateria_callback, 10)
    ...
    def bateria_callback(self, msg):
        with self.lock:
            if msg.data < 20.0 and self.estado != 'emergencia':
                ...
                self.get_logger().warn('BATERIA CRÍTICA: emergencia')
